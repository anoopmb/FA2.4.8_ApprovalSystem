...
function display_gl_heading($myrow)
{
    ...
    switch ($myrow["status"]) {
        ...
        case 0:
            label_row(_('Approved By'), "Approval Pending", "class='tableheader2'", "colspan=6 style='color:orange;'");
            break;
        case 1:
            $details = get_approval_details($_GET['type_id'], $_GET['trans_no']);
            if ($details != null) {
                label_row(_('Approved By'), get_user_real_name($details['user_id']) . " on " . sql2date($details['tran_date']), "class='tableheader2'", "colspan=6 ");
            } else
                label_row(_('Approved By'), $myrow["real_name"], "class='tableheader2'", "colspan=6 ");
            break;

        case 2:
            $details = get_approval_details($_GET['type_id'], $_GET['trans_no']);
            if ($details != null) {
                label_row(_('Approved By'), "Rejected By " . get_user_real_name($details['user_id']), "class='tableheader2'", "colspan=6 style='color:red;' ");
                if (strlen($details['memo_']) > 0)
                    label_row(_("Memo for Rejection"), nl2br($details['memo_']), "class='tableheader2'", "colspan=6 style='color:red;'");

            } else
                label_row(_('Approved By'), "Rejected", "class='tableheader2'", "colspan=6 style='color:red;'");
            break;
    }
...
$result = get_gl_trans($_GET['type_id'], $_GET['trans_no'], false);
